#!/usr/bin/env bash
#
# Deterministic command-line build for Dicee.
#
...
PROJECT="Dicee.xcodeproj"
SCHEME="Dicee"

# Pinned simulator. Repin via DICEE_SIM; resolution is by name so it stays
# stable across machines even when the runtime gets a patch update.
DEVICE_NAME="${DICEE_SIM:-iPhone 17}"
...
ensure_booted() {
  ...
  if [ -z "$udid" ]; then
    echo "error: simulator '$DEVICE_NAME' not found. Set DICEE_SIM or create it." >&2
    exit 1
  fi
